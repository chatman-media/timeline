...
export const ResizableVideo = React.memo(
  function ResizableVideo({
    ...
    // Используем более редкую синхронизацию, чтобы не создавать задержек
    useEffect(() => {
      if (!videoRef.current) return
      ...
      const handleTimeUpdate = () => {
        ...
        // Остальные видео синхронизируются с ним, но только при значительном расхождении
        if (index === 0 && videoRefs) {
          // Ограничиваем частоту обновлений - увеличиваем интервал для уменьшения нагрузки
          const now = Date.now()
          if (now - lastUpdateTimeRef.current < updateIntervalMs * 2) {
            return
          }
          ...
          // Синхронизируем время только при значительном расхождении (более 0.5 секунды)
          Object.entries(videoRefs).forEach(([id, element]) => {
            if (
              id !== video.id &&
              element &&
              !element.paused &&
              Math.abs(element.currentTime - currentTime) > 0.5 // Увеличиваем порог синхронизации
            ) {
              try {
                // Используем requestAnimationFrame для более плавной синхронизации
    ...
    }, [video.id, videoRefs, index, lastUpdateTimeRef, updateIntervalMs])
              ...
              if (target) {
                if (target.videoWidth === 0 || target.videoHeight === 0) {
                  ...
                  if (isPlaying && target.paused) {
                    console.log(
                      `[ResizableVideo] Автоматический запуск видео ${video.id} после загрузки`,
                    )
