...
const initMediaStream = useCallback(async () => {
  try {
    ...
    if (captureMode === "screen" && includeCamera && cameraStreamRef.current && cameraVideoRef.current) {
      cameraVideoRef.current.srcObject = cameraStreamRef.current
      cameraVideoRef.current.muted = true
      
      cameraVideoRef.current.onloadedmetadata = () => {
        cameraVideoRef.current?.play().catch(e => console.error("Ошибка воспроизведения камеры:", e))
      }
    }
...
const startRecording = useCallback(() => {
  ...
  if (recordBothStreams && cameraStreamRef.current) {
    try {
      ...
      try {
        cameraRecorderRef.current = new MediaRecorder(cameraStreamRef.current, { mimeType: "video/webm" })
      } catch (e) {
  ...
  mediaRecorderRef.current.onstop = () => {
    ...
    if (recordBothStreams && cameraRecorderRef.current && cameraRecorderRef.current.state !== "inactive") {
