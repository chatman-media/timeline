    ...
    try {
      ...
      if (error instanceof DOMException) {
        if (error.name === "NotAllowedError") {
          setErrorMessage("Доступ к камере и микрофону запрещен. Пожалуйста, разрешите доступ в настройках браузера.")
          setPermissionStatus("denied")
        } else if (error.name === "NotFoundError") {
          setErrorMessage("Камера или микрофон не найдены. Пожалуйста, подключите устройства и попробуйте снова.")
        } else {
        ...
        if (capabilities.width && capabilities.height) {
          ...
          if (capabilities.frameRate) {
            ...
            if (frMin !== undefined && frMax !== undefined) {
              ...
              frameRates = standard.filter(
                fps => fps >= frMin && fps <= frMax
              )
      ...
      <DialogContent className="sm:max-w-[500px] bg-[#18181B] text-white border-[#333] p-0 overflow-hidden">
        ...
        <div className="p-4">
          ...
          {permissionStatus === "pending" && (
            <div className="text-sm mb-4 text-center">Запрашиваем доступ к камере и микрофону...</div>
          )}
          ...
          {permissionStatus === "granted" && (
            <>
              ...
              <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-4 items-center mb-6">
                ...
                <Select value={selectedDevice} onValueChange={setSelectedDevice} disabled={isRecording || isLoadingCapabilities}>
                ...
                <Select value={selectedAudioDevice} onValueChange={setSelectedAudioDevice} disabled={isRecording || isLoadingCapabilities}>
                  <SelectTrigger className="bg-[#222] border-[#444] focus:ring-offset-0 focus:ring-0">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-[#222] border-[#444]">
                ...
                <div>
                  {isLoadingCapabilities ? (
                    ...
                    <Select value={selectedResolution} onValueChange={setSelectedResolution} disabled={isRecording}>
                      <SelectTrigger className="bg-[#222] border-[#444] focus:ring-offset-0 focus:ring-0">
                        <SelectValue />
                      </SelectTrigger>
              ...
              <div className="flex flex-col items-center">
                {!isRecording ? (
                  <Button 
                    className="w-20 h-20 rounded-full bg-red-500 hover:bg-red-600 flex items-center justify-center mb-4 border-2 border-white"
        ...
        <div className="p-4 flex justify-end border-t border-[#333]">
          <Button
            className="bg-[#0CC]  hover:bg-[#0AA] text-black"
            onClick={handleClose}
          >
            OK
          </Button>
